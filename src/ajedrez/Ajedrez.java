...
public class Ajedrez {
    ...
    /* metodo para visualizar tablero por terminal 
    public static void boardVisualization(String[][] board) {
 ...
 /* metodo para visualizar tablero de opciones posibles 

    public static void optionsVisualization(int[][] options, String[][] board) {
...
// metodo para selecionar la pieza deseada 

    public static void selectPiece(int[] piecePosition, String[][] board) {

        int row, column;
        boolean correctPiece, insideBoard;

        do {
            correctPiece = false;
            insideBoard = false;
            // pedimos al usuario  introduzca fila y columna 
            System.out.println("Select the piece you are going to move: ");
            System.out.print("Row: ");
            row = sc.nextInt();
            System.out.print("Column: ");
            column = sc.nextInt();
            //indicamos si ha elegido una opcion no valida = fuera de tablero 
            if (row <= 0 || row > MAX_ROW || column <= 0 || column > MAX_COLUMN) {
                System.out.println("Opcion no valida!");
            } else {
                insideBoard = true;
                //indicamos si se ha elegido espacio donde no hay piezas 
                if (board[row - 1][column - 1].equals(" . ")) {
                    System.out.println("There is not piece!");
                } else {
                    correctPiece = true;
                }
            }
        } while (!correctPiece || !insideBoard); // repetimos hasta seleccionar una pieza 
        //guardamos la posicion en un array para poderlo utilizar en el main 
        piecePosition[0] = row - 1;
        piecePosition[1] = column - 1;
    }

    //metodo para seleccionar la nueva posicion de la pieza 
    public static void selectNewPosition(int[] position, int[][] validOptions) {
        boolean insideBoard, correctPosition;

        do {
            insideBoard = false;
            correctPosition = false;
            //pedimos al usuario que inique en el tablero donde quiere colocar la pieza
            System.out.println("Select where you are going to move: ");
            System.out.print("Row: ");
            position[0] = sc.nextInt() - 1;
            System.out.print("Column: ");
            position[1] = sc.nextInt() - 1;

            if (position[0] <= 0 || position[0] > MAX_ROW || position[1] <= 0 || position[1] > MAX_COLUMN) {
                System.out.println("There is not a correct option!");
            } else {
                insideBoard = true;
                //indicamos si se ha elegido espacio donde no hay piezas 
                if (validOptions[position[0]][position[1]] != 1) {
                    System.out.println("There is not a correct option!");
                } else {
        ...
        } while (!correctPosition || !insideBoard); // mientras sea una opcion no valida, es decir en la matriz de opciones validas haya un 1 en esa posicion 
    }
     */
